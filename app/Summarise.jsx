...
import { PDF_BUCKET_ID } from 'expo-constants'
...
const Summarise = () => {
  ...
  const storeFileInAppwrite = async () => {
    try {
      const response = await storage.createFile('67bccd990005a5d175c4', ID.unique(), file);
  ...
  const uploadFile = async () => {
    try {
      ...
      const url = storage.getFileDownload('67bccd990005a5d175c4', fileID);
